...
Given an issue labeled `agent-ready`, when the polling agent runs (hourly on the Mac Mini, or is triggered manually), then it claims the issue, does the work, and opens a PR that references the issue.
...
- **FR-007**: Model routing MUST default to Anthropic models for all agents in this iteration, and MUST be architected so multi-model routing (including local models) can be introduced later without a workflow redesign. (Superseded for the reviewer role specifically: `omnigent-setup.md` FR-010, decided 2026-08-14, requires Codex as the pinned cross-vendor reviewer — see Scenario 3 above and FR-011 below. This bullet's "Anthropic for all agents" was written before that decision and was never updated; Codex-as-reviewer is the actual current default, not a contradiction to resolve later.)
...
- An hourly polling cadence (Omnigent's scheduled-task platform floor, confirmed 2026-08-14) is acceptable latency for now; the manual-trigger path covers on-demand checks. Revisit if hourly becomes a bottleneck once W1 execution starts.
